...
tab
library(stringr)
str_detect(tab$Player, "AUS/ICC")
dim(tab)
table(str_detect(tab$Player, "IND"))
View(tab)

ind <- tab[str_detect(tab$Player, "IND")==TRUE,]
tab[str_detect(tab$Player, "AUS")==TRUE,]
...
ind <- tab %>% 
  ...
  summarise(numbers = n(),ninns = sum(Inns),runs <- sum(Runs), avg = mean(Ave),n100 = sum(`100`), n50 = sum(`50`), n_0 = sum(`0`), avg_100 = sum(`100`)/sum(Inns)*100,avg_50 = sum(`50`)/sum(Inns)*100,avg_0 = sum(`0`)/sum(Inns)*100 )
ind
...
TABLE <- rbind(ind,aus, eng, arf, sri, pak, wi,ban,nz)
TABLE
TABLE <- as.data.frame(TABLE)
rownames(TABLE) <- c("INDIA", "AUSTRALIA","ENGLAND","AFRICA","SRILANKA","PAKISTAN", "WESTINDIES","BANGLADESH", "NEWZELAND")
TABLE
final_table <- TABLE %>% 
  arrange(-numbers)
